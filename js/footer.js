document.write(`
    <section id="footerZ" style="margin-top:45px;">
		<div class="" style="font-family: Roboto;">
			<div class="row">
				<div class="col-sm-12 col-lg-3" style="text-align: center;">
					<img src="/img/branding.png" style="width: 200px; height: 200px;"/>
					<h2 style="position: relative; top: -45px;"><b>masha</b> <span style="color: #f26522; font-weight: 500;">art</span></h2>
				</div>

				<div class="col-sm-12 col-lg-3">
					<h4>Gallery</h4>
					<a href="Gallery.html?type=paintings" class="sublink">Paintings</a><br />
					<a href="Gallery.html?type=sculptures" class="sublink">Sculptures</a><br />
					<a href="Gallery.html?type=handicrafts" class="sublink">Handicrafts</a><br />
					<h4>Artists</h4>
					<a href="Gallery.html?type=artists" class="sublink">Featured artists</a><br />
					<a href="Gallery.html?type=artists#allArtistsContainer" class="sublink">All artists</a><br />
					<h4>Showcase</h4>
					<a href="events.html?type=events" class="sublink">Upcoming events</a><br />
					<a href="events.html?type=exhibitions" class="sublink">Exhibitions</a><br />
				</div>
				<div class="col-sm-12 col-lg-3">
					<h4><a href="#">Masha</a></h4>
					<a href="about.html" class="sublink">About</a><br />
					<a href="inmedia.html" class="sublink">In Media</a><br />
					<h4>Masha Support</h4> 
					<a class="sublink" href="masha_support.html">About Masha Support</a><br />
					<h4>Social</h4>
					<a href="http://facebook.com/mashaesthetics" class="sublink"><i class="fa fa-facebook"></i> Facebook</a><br />
					<a href="http://instagram.com/mashaesthetics" class="sublink"><i class="fa fa-instagram"></i> Instagram</a><br />
					<a href="https://linkedin.com/company/masha-art/" class="sublink"><i class="fa fa-linkedin"></i> LinkedIn</a><br />
				</div>
				<div class="col-sm-12 col-lg-3">
					<h4>Contact</h4>
					<p style="padding-left: 20px;">#212/214, M3M Cosmopolitan<br />
					Sector 66; Gurugram, Haryana 12210</p>
					<a class="sublink" href="https://maps.app.goo.gl/zH765GrPdPjKEuLd7"><i class="glyphicon glyphicon-map-marker"></i> Find us on Google Maps</a><br />
					<br />
					<a class="sublink" href="tel:+918948799799"><i class="glyphicon glyphicon-earphone"></i> +91-8948799799</a><br />
					<a class="sublink" href="tel:+919999636199"><i class="glyphicon glyphicon-earphone"></i> +91-9999636199</a><br />
				</div>
			</div>

            <br />

            <div class="row justify-content-center">
                <div class="col-8 text-center">
                    <p>&copy; Copyright 2019 <strong>Masha Art</strong> <span style="padding: 0px 5px">&middot;</span> Designed by <strong>Cryptina India</strong></p>
                </div>
            </div>

		</div>
	</section>
`);